...
| Application Load Balancer	 | Yes - Global only	 | Yes - Regional | 
| Network Load Balancer (non-HTTP traffic) | 	Yes - Global or regional | 	Yes - Regional | 
| Internal load balancers	 | Yes - Regional VIP (+ client can be anywhere)	 | No | 
| Google Cloud Storage, Google Kubernetes Engine	 | Yes | 	Yes - Regional but only by using load balancing | 
| Cloud CDN	 | Yes | 	No | 
